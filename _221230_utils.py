...
class MyModel(tf.keras.Model):
	...
	def generate_text(self, seed='Herein, we describe a new model for', length=1000):
		one_step_model = OneStep(self, CharMapping.chars_from_ids, CharMapping.ids_from_chars)
		states = None
...
def generate_dataset(start_index, stop_index, path_to_file = 'abstracts.txt'):
	...
	# the `text` variable must be overwritten so that my RAM doesn't explode
	# note that this happens after `vocab_size` is set so that the model retains a constant vocab
